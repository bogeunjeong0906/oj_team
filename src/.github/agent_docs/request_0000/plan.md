...
## Replacement Scope and Cleanup Plan

- 유지할 활성 경로
- 대체 대상 경로
- 삭제 후보 경로
- 유지가 필요한 레거시 경로와 그 이유
...
3. 필요한 수정 순서를 결정한다.
4. obsolete path 제거 여부와 예외를 결정한다.
5. 각 수정 후 검증 방법을 정한다.
6. 완료 기준과 롤백 조건을 정한다.
...
- 경계 조건 테스트
- 회귀 방지 테스트
- 각 테스트 또는 검증이 어떤 링크를 확인하는지
- mock이나 fixture가 필요한 경우 고정할 링크와 이유

## Complexity Guardrails

- 이번 변경에서 늘려도 되는 구조와 늘리면 안 되는 구조
- 새 helper, branch, shim이 필요한 경우 그 이유
- 기대하는 활성 경로 수 변화: 감소 | 유지 | 증가
...
- 남아 있는 불확실성이 기록된다.
- replacement scope와 cleanup scope가 명시된다.
- 테스트 설계가 어느 링크를 검증하는지 설명한다.
